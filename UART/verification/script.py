...
Version mejorada:
- Soporta operandos en hex (0x..) y binario (0b..), no solo decimal
- Deteccion automatica de puertos seriales
- Salida en terminal con colores/tablas (rich)
- Limpieza de buffer antes de cada transaccion
- Muestra resultado de add/sub tambien en signed (int8)
"""
...
class ALUController:
    # Mapeo de operaciones a codigos binarios (funct field estilo MIPS)
    OPERATIONS = {
        'add': 0x20,  # 100000
        'sub': 0x22,  # 100010
        'and': 0x24,  # 100100
    ...
    def execute_operation(self, operation, operand_a, shift_or_b):
        ...
        if operation in self.SHIFT_OPS:
            if not (0 <= shift_or_b <= 3):
                console.print("[red]✗ Cantidad de desplazamiento debe ser 0-3[/red]")
                return None
            # Codificar la cantidad de shift en los bits [7:6] del segundo byte
            operand_b_encoded = shift_or_b << 6
        else:
            if not (0 <= shift_or_b <= 255):
                console.print("[red]✗ Operando B fuera de rango (0-255)[/red]")
                return None
            operand_b_encoded = shift_or_b

        if not (0 <= operand_a <= 255):
    ...
    def parse_and_execute(self, command):
        ...
        if operation in ('add', 'sub'):
            ...
            console.print(
                f"[bold green]← Resultado:[/bold green] {result} (uint8) / {signed} (int8) "
                f"= 0x{result:02X}"
            )
        elif operation in ('and', 'or', 'xor', 'nor'):
            console.print(
                f"[bold green]← Resultado:[/bold green] {result} = 0b{result:08b} = 0x{result:02X}"
            )
        elif operation in self.SHIFT_OPS:
            console.print(
                f"[bold green]← Resultado:[/bold green] {result} = 0b{result:08b}  "
                f"(valor {operand_a} desplazado {operand_b} posiciones a la derecha)"
            )
    ...
    def show_help(self):
        ...
        rows = [
            ("ADD", "Suma", "add 10 20"),
            ("SUB", "Resta", "sub 50 30"),
            ("AND", "AND logico", "and 0xFF 0x0F"),
            ("OR", "OR logico", "or 128 64"),
            ("XOR", "XOR logico", "xor 170 85"),
            ("NOR", "NOR logico", "nor 15 240"),
            ("SRL", "Shift Right Logical (0-3 bits)", "srl 128 2"),
            ("SRA", "Shift Right Arithmetic (0-3 bits)", "sra 128 3"),
        ]
        for r in rows:
            table.add_row(*r)
        console.print(table)
